...
DOMUpdater.prototype.getID = function(newTag, markCache) {
  ...
  var currentTagID = _getTagIDAtDocumentPos(
    this.editor,
    ...
    marks.forEach(function(mark) {
      if (mark.hasOwnProperty('tagID') && nodeMap[mark.tagID]) {
        ...
        if (
          !(
            markInfo &&
            _posEq(markInfo.range.from, node.startPos) &&
            _posEq(markInfo.range.to, node.endPos)
          )
        ) {
          mark.clear()
 ...
 */
DOMUpdater.prototype._handleDeletions = function(
  nodeMap,
  oldSubtreeMap,
  newSubtreeMap
) {
  ...
  if (deletedIDs.length) {
    ...
    marks.forEach(function(mark) {
      if (
        mark.hasOwnProperty('tagID') &&
        deletedIDs.indexOf(mark.tagID) !== -1
      ) {
        mark.clear()
      }
    })

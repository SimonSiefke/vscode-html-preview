...
test('unclosed-p-tag', async () => {
	const uri = await createTestFile('unclosed-p-tag.html')
  await setText(`<p>hello`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/unclosed-p-tag.html', {waitUntil: 'networkidle2', timeout: 15000})
  // await new Promise(resolve => setTimeout(resolve, 444000))
	...
	assert.equal(adjust(html), `<html><head></head><body><p>hello</p></body></html>`);

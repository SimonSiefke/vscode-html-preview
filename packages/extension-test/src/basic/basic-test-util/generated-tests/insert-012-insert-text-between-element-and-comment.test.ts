...
test('insert-012-insert-text-between-element-and-comment', async () => {
	const uri = await createTestFile('insert-012-insert-text-between-element-and-comment.html')
  await setText(`<h1>a</h1><!--c-->`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/insert-012-insert-text-between-element-and-comment.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body><h1>a</h1>b<!--c--></body></html>`);

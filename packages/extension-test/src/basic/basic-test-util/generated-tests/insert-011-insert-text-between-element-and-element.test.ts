...
test('insert-011-insert-text-between-element-and-element', async () => {
	const uri = await createTestFile('insert-011-insert-text-between-element-and-element.html')
  await setText(`<h1>a</h1><h1>c</h1>`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/insert-011-insert-text-between-element-and-element.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body><h1>a</h1>b<h1>c</h1></body></html>`);

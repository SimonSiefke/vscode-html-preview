...
test('open-implies-close-1', async () => {
	const uri = await createTestFile('open-implies-close-1.html')
  await setText(`<ruby>Text goes here<rt>annotation goes here</ruby>`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/open-implies-close-1.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body><ruby>Text goes here<rt>annotation goes here!</rt></ruby></body></html>`);

...
test('pre', async () => {
	const uri = await createTestFile('pre.html')
  await setText(` <pre>
body {
color: red;
}
  ...
  await page.goto('http://localhost:3000/pre.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body><pre>body {
color: orangered;
}
</pre></body></html>`);

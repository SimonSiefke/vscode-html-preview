...
test('insert-020-insert-text-between-comment-and-text', async () => {
	const uri = await createTestFile('insert-020-insert-text-between-comment-and-text.html')
  await setText(`<!--a-->c`)
  ...
  await page.goto('http://localhost:3000/insert-020-insert-text-between-comment-and-text.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body>bc</body></html>`);

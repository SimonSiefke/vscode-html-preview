...
test('textarea', async () => {
	const uri = await createTestFile('textarea.html')
  await setText(`<textarea name="buffer" rows=25 cols=80 readonly>
...
[ -z "$PS1" ] &amp;&amp; return
  ...
  await page.goto('http://localhost:3000/textarea.html', {waitUntil: 'networkidle2', timeout: 15000})
...
await waitForUpdateEnd(page)
	...
	assert.equal(adjust(html), `<html><head></head><body><textarea name="buffer" rows="25" cols="80" readonly=""># If not running interactively, don't do anything
[ -z "$PS1" ] &amp;&amp; return;

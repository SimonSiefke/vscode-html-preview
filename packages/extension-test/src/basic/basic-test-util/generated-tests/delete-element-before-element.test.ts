...
test('delete-element-before-element', async () => {
	const uri = await createTestFile('delete-element-before-element.html')
  await setText(`<h1>a</h1><h1>b</h1>`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/delete-element-before-element.html', {waitUntil: 'networkidle2', timeout: 15000})

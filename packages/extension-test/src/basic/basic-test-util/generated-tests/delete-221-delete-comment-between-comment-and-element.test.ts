...
test('delete-221-delete-comment-between-comment-and-element', async () => {
	const uri = await createTestFile('delete-221-delete-comment-between-comment-and-element.html')
  await setText(`<!--a--><!--b--><h1>c</h1>`)
  await activateExtension()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  const browser = await getBrowser()
  const page = await browser.newPage()
  // await new Promise(resolve => setTimeout(resolve, 1000))
  await page.goto('http://localhost:3000/delete-221-delete-comment-between-comment-and-element.html', {waitUntil: 'networkidle2', timeout: 15000})

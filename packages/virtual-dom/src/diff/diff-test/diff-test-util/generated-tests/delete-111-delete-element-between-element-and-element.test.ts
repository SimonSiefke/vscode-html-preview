...
test(`delete-111-delete-element-between-element-and-element.test.txt`, () => {
  let offsetMap = Object.create(null)

  let id = 0
  const p1 = parse(`<h1>a</h1><h1>b</h1><h1>c</h1>`, offset => {
    const nextId = id++
  ...
  const p2 = parse(`<h1>a</h1><h1>c</h1>`, (offset, tokenLength) => {
    let nextId: number

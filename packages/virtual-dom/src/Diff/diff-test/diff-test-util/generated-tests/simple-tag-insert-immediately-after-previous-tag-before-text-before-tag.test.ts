...
test(`simple-tag-insert-immediately-after-previous-tag-before-text-before-tag.test.txt`, () => {
  let offsetMap = Object.create(null)

  let id = 0
  const p1 = parse(`<h1>a</h1>
<h2>b</h2>`, offset => {
    const nextId = id++
  ...
  const p2 = parse(`<h1>a</h1><div>New Content</div>
<h2>b</h2>`, (offset, tokenLength) => {
    let nextId: number

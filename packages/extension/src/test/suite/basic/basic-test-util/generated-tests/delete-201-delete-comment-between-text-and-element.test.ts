...
test('delete-201-delete-comment-between-text-and-element', async () => {
	const uri = await createTestFile('delete-201-delete-comment-between-text-and-element.html')
  await setText(`a<!--b--><h1>c</h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/delete-201-delete-comment-between-text-and-element.html', {waitUntil: 'networkidle2', timeout: 10000})

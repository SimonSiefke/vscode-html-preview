...
test('replace-text-with-element', async () => {
	const uri = await createTestFile('replace-text-with-element.html')
  await setText(`h1`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/replace-text-with-element.html', {waitUntil: 'networkidle2', timeout: 10000})

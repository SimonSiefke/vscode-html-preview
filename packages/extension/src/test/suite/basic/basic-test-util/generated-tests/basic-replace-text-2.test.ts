...
test('basic-replace-text-2', async () => {
	const uri = await createTestFile('basic-replace-text-2.html')
  await setText(`<h1>aa</h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/basic-replace-text-2.html', {waitUntil: 'networkidle2', timeout: 15000})
  //await page.goto('http://localhost:3000/basic-replace-text-2.html')

...
test('delete-002-delete-text-between-text-and-comment', async () => {
	const uri = await createTestFile('delete-002-delete-text-between-text-and-comment.html')
  await setText(`ab<!--c-->`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/delete-002-delete-text-between-text-and-comment.html', {waitUntil: 'networkidle2', timeout: 10000})
  //await page.goto('http://localhost:3000/delete-002-delete-text-between-text-and-comment.html')

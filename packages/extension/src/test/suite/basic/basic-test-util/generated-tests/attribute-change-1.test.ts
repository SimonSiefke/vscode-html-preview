...
test('attribute-change-1', async () => {
	const uri = await createTestFile('attribute-change-1.html')
  await setText(`<h1 class></h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/attribute-change-1.html', {waitUntil: 'networkidle2', timeout: 10000})
  //await page.goto('http://localhost:3000/attribute-change-1.html')

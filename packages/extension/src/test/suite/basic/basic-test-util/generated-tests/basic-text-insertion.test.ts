...
test('basic-text-insertion', async () => {
	const uri = await createTestFile('basic-text-insertion.html')
  await setText(`<h1><p>ok</p></h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/basic-text-insertion.html', {waitUntil: 'networkidle2', timeout: 15000})
  //await page.goto('http://localhost:3000/basic-text-insertion.html')

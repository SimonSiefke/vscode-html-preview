...
test('insert-202-insert-text-between-comment-and-text', async () => {
	const uri = await createTestFile('insert-202-insert-text-between-comment-and-text.html')
  await setText(`<!--a-->c`)
  ...
  vscodeEdit.replace(
	...
	assert.equal(adjust(html), `<!--a--><html><head></head><body>bc</body></html>`);

...
test('delete-text-before-element', async () => {
	const uri = await createTestFile('delete-text-before-element.html')
  await setText(`a<h1>b</h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/delete-text-before-element.html', {waitUntil: 'networkidle2', timeout: 10000})
  //await page.goto('http://localhost:3000/delete-text-before-element.html')

...
test('element-addition-at-the-start', async () => {
	const uri = await createTestFile('element-addition-at-the-start.html')
  await setText(`<h1>a</h1>`)
  await activateExtension()
  const browser = await getBrowser()
  const page = await browser.newPage()
  await vscode.commands.executeCommand('htmlPreview.openPreview')
  await page.goto('http://localhost:3000/element-addition-at-the-start.html', {waitUntil: 'networkidle2', timeout: 15000})
  //await page.goto('http://localhost:3000/element-addition-at-the-start.html')

...
import { findCssSelectorAtOffset } from '@htmlpit/html-preview-service/dist/findCssSelectorAtPosition/findCssSelectorAtPosition'
import { urlParseQuery, urlParseHtmlPathname } from '@htmlpit/html-preview-service-node'
...
export const localPluginHighlight: LocalPlugin = api => {
  ...
  const onDidChangeCssTextEditor = (event: vscode.TextEditorSelectionChangeEvent): void => {
    if (event.textEditor.document.languageId !== 'css') {
      return
    }
    ...
    const cssSelectorAtOffset = findCssSelectorAtOffset(text, offset)
    if (!cssSelectorAtOffset) {
      return
    }
    ...
    api.webSocketServer.broadcast({
      commands: [
        {
          command: 'highlightSelector',
          payload: {
            selector: cssSelectorAtOffset,
          },
        },
      ],
    })
  ...
  const onDidChangeHtmlTextEditor = (event: vscode.TextEditorSelectionChangeEvent): void => {
  ...
  vscode.window.onDidChangeTextEditorSelection(event => {
    if (event.textEditor.document.languageId === 'css') {
      return onDidChangeCssTextEditor(event)
    }
    if (event.textEditor.document.languageId === 'html') {
      return onDidChangeHtmlTextEditor(event)
    }
  })
}
